import pytest
from sudoku import SudokuGrid, InvalidSudokuInput
import numpy as np
...
def test_default_grid_values():
    ...
    for row in range(1,ROWS):
        for column in range(1,COLUMNS):
            assert grid.get_grid()[column][row] == [0], "every entry should be zero"
...
def test_parsed_sudoku_values_all_ones():
    ...
    for row in range(1,ROWS):
        for column in range(1,COLUMNS):
            assert grid.get_grid()[column][row] == [1], "every entry should be one"
...
def test_parsed_sudoku_values_asc_in_row():
    ...
    assert len(sudoku) == ROWS*COLUMNS, "Sudoku field has 81 cells"
    assert grid.get_grid()[0][0] == [1] and grid.get_grid()[1][0] == [2] and grid.get_grid()[2][0] == [3] and grid.get_grid()[8][0] == [9], "first row should have the values 1,2,...,9"
    assert grid.get_grid()[0][0] == [1] and grid.get_grid()[0][1] == [1] and grid.get_grid()[0][2] == [1] and grid.get_grid()[0][8] == [1], "first column should have the value 1"
...
def test_parsed_sudoku_short_string():
    ...
    assert len(sudoku_to_short) == 79, "Sudoku string should have 79 cells, because its to short"

    with pytest.raises(InvalidSudokuInput, match="Length or parsed sudoku needs to be {}, but is {}".format(ROWS*COLUMNS,len(sudoku_to_short))):
...
def test_parsed_sudoku_long_string():
    ...
    assert len(sudoku_to_long) == 82, "Sudoku string should have 82 cells, because its to short"
    
    with pytest.raises(InvalidSudokuInput, match="Length or parsed sudoku needs to be {}, but is {}".format(ROWS*COLUMNS,len(sudoku_to_long))):
...
def test_parsed_sudoku_special_character():
    ...
    with pytest.raises(InvalidSudokuInput, match="Parsed sudoku contains character that is not a digit"):
        SudokuGrid(sudoku_to_long)
